...
describe("LastReportDateService Unit Tests", () => {
    ...
    it("should call find and only return the authID", async () => {
        ...
        jest.spyOn(LastReportDateModel.getModel, "find").mockResolvedValue(mockData);

        const result = await service.getAuthID();
        expect(LastReportDateModel.getModel.find).toHaveBeenCalledWith({authID: {$exists: true}});
        expect(result).toEqual(mockData);
    });
    ...
    it("createOrUpdateLastReportDate should be called with correct model object", async () => {
        ...
        jest.spyOn(global.crypto, "randomUUID").mockReturnValue("mocked-uuid-for-testing-1234");
        
        const findOneAndUpdateSpy = jest.spyOn(LastReportDateModel.getModel, "findOneAndUpdate").mockResolvedValue(null);
        ...
        expect(findOneAndUpdateSpy).toHaveBeenCalledWith({}, modelObj, {upsert: true});
