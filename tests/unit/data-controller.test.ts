...
describe("DataController", () => {
    ...
    it("should return all flock cases successfully", async () => {
        ...
        jest.spyOn(FlockCasesByStateService.prototype, "getAllFlockCases").mockResolvedValue(mockData);        
        await dataController.getAllFlockCases(req as Request, res as Response);
        expect(FlockCasesByStateService.prototype.getAllFlockCases).toHaveBeenCalled();
    ...
    it("should handle errors when fetching flock cases", async () => {
        jest.spyOn(FlockCasesByStateService.prototype, "getAllFlockCases").mockRejectedValue(new Error("DB error"));
        await dataController.getAllFlockCases(req as Request, res as Response);
        expect(FlockCasesByStateService.prototype.getAllFlockCases).toHaveBeenCalled();
        expect(statusMock).toHaveBeenCalledWith(500);
        expect(jsonMock).toHaveBeenCalledWith({ error: "Failed to fetch last report date" });
    });

    it("should return US summary successfully", async () => {
        const mockData = [ USSummaryModel.getModel.hydrate({
        ...
        jest.spyOn(USSummaryService.prototype, "getUSSummary").mockResolvedValue(mockData);
    ...
    it("should handle errors when fetching US summary", async () => {
        jest.spyOn(USSummaryService.prototype, "getUSSummary").mockRejectedValue(new Error("DB error"));
        await dataController.getUSSummary(req as Request, res as Response);
        expect(USSummaryService.prototype.getUSSummary).toHaveBeenCalled();
        expect(statusMock).toHaveBeenCalledWith(500);
        expect(jsonMock).toHaveBeenCalledWith({ error: "Failed to US Summary!" });
    });

    it("should return last report date successfully", async () => {
        const mockData = [ LastReportDateModel.getModel.hydrate({
            "lastScrapedDate": {
        ...
        jest.spyOn(LastReportDateService.prototype, "getLastReportDate").mockResolvedValue(mockData);
        await dataController.getLastReportDate(req as Request, res as Response);
        expect(LastReportDateService.prototype.getLastReportDate).toHaveBeenCalled();
        expect(statusMock).not.toHaveBeenCalled();
        expect(jsonMock).toHaveBeenCalledWith(mockData);
    });

    it("should handle errors when fetching last report date", async () => {
        jest.spyOn(LastReportDateService.prototype, "getLastReportDate").mockRejectedValue(new Error("DB error"));
        await dataController.getLastReportDate(req as Request, res as Response);
        expect(LastReportDateService.prototype.getLastReportDate).toHaveBeenCalled();
        expect(statusMock).toHaveBeenCalledWith(500);
        expect(jsonMock).toHaveBeenCalledWith({ error: "Failed to fetch last report date!" });
